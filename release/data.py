...
class GeneratorData(object):
    ...
    def get_batch(self, batch_size, smiles_augmentation=None):
        """
        Get a batch of training sequences for efficient transformer training.
        
        Parameters
        ----------
        batch_size: int
            Number of sequences in the batch
        smiles_augmentation: SmilesEnumerator (optional)
            SMILES augmentation object
            
        Returns
        -------
        inputs: torch.Tensor
            Batched input sequences [batch_size, max_seq_len]
        targets: torch.Tensor  
            Batched target sequences [batch_size, max_seq_len]
        attention_mask: torch.Tensor
            Attention mask for variable-length sequences [batch_size, max_seq_len]
        """
        sequences = []
        max_len = 0
        ...
        for i, seq in enumerate(sequences):
            ...
            for j in range(seq_len):
                inputs[i, j] = self.all_characters.index(seq[j])
            # Target: all tokens except first  
            for j in range(seq_len):
                targets[i, j] = self.all_characters.index(seq[j + 1])
